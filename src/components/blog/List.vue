<template>
  <section class="flex flex-1 flex-col">
    ...
    <ul
      v-if="articleList.length > 0"
      class="article-list md:mx-0 mx-4"
    >
      <li
...
const getCategoryList = (): void => {
  ...
  categoryList.value = categories.map( item => {
    return {
      routeName: lang.value === defaultLang ? `category-${item.path}-page` : `${lang.value}-category-${item.path}-page`,
      path: lang.value === defaultLang ? `/category/${item.path}` : `/${lang.value}/category/${item.path}`,
  ...
  categoryList.value.unshift({
    routeName: lang.value === defaultLang ? 'article-page' : `${lang.value}-article-page`,
...
const getPageInfo = (): void => {
  ...
  routes.value = router.getRoutes()
    .filter( item => {
      ...
      // 判断当前是否分类列表
      const CATEGORY_PATH: string = lang.value === defaultLang ? '/category' : `/${lang.value}/category`;
