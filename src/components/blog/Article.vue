  ...
  <section class="flex justify-start flex-col flex-1 md:w-auto w-full overflow-hidden">
    ...
    <p class="flex items-center w-full text-sm text-gray-400 md:mb-8 mb-4 md:pb-8 pb-4 border-b dark:border-white dark:border-opacity-5">
      ...
      <span :title="date.substr(0, 10)">
        {{ days > 7 ? date.substr(0, 10) : dateAgo }}
      </span>
    </p>
    <!-- 发布信息 -->
    ...
    >
      本文最后更新于 {{ days }} 天前，部分内容可能不适合当前所有情况，仅供参考。
    </p>
    <!-- 过期提示 -->
...
export default defineComponent({
  ...
  setup (props) {
     ...
     * 计算日期的显示
     */
    const getDateInfo = (): void => {
      // 计算发布日期到今天的日期差
      const nowDate = dayjs(Date.now());
      const createDate = dayjs(+new Date(date));
      days.value = nowDate.diff(createDate, 'day');

      // 计算发布时间是X天前
      dateAgo.value = dayjs(createDate).fromNow();
    }
